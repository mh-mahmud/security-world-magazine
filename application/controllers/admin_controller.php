...
class Admin_controller extends CI_Controller {
	...
	*/
	...
	public function add_person($data=null) {
		...
		#duplicate test
		$this->webspice->db_field_duplicate_test("SELECT * FROM persons WHERE PERSON_TYPE=? AND NAME=? AND DETAILS=?", array($input->person_type, $input->name, $input->details), 'You are not allowed to enter duplicate person', 'PERSON_ID', $input->person_id, $data, 'admin/add_person');
		...
		$this->webspice->process_image_single('images',$this->db->insert_id(), 'person_full');
	...
	public function add_images() {
		...
		$sql = "
		INSERT INTO gallery
		(CAT_ID, SUB_CAT_ID, IMAGE_CAPTION, IMAGE_LINK, CREATED_BY, CREATED_DATE, STATUS)
		VALUES
		( ?, ?, ?, ?, ?, ?, 7)";
		$this->db->query($sql, array($input->cat_id, $input->sub_cat_id, $input->image_caption, $_FILES['image_link']['name'], $this->webspice->get_user_id(),$this->webspice->now()));
	...
	public function create_page($data=null) {
		...
		$this->load->library('form_validation');
		$this->form_validation->set_rules('sub_category_id','main page name','trim|xss_clean');
		$this->form_validation->set_rules('sub_sub_category_id','page name','trim|xss_clean');
		...
		if( $input->page_id ){
			...
			$this->db->query($sql);*/
			$sql = "
			UPDATE pages SET SUB_CATEGORY_ID=?, SUB_SUB_CATEGORY_ID=?, PAGE_TITLE=?, PAGE_DETAILS=?, UPDATED_BY=?,UPDATED_DATE=?
			WHERE PAGE_ID=?";
			$this->db->query($sql, array($input->sub_category_id, $input->sub_sub_category_id, $_POST['page_title'], $_POST['page_details'], $this->webspice->get_user_id(), $this->webspice->now(), $input->page_id));
			$this->webspice->message_board('Record has been updated!');
			$this->webspice->log_me('data_updated - '.$this->webspice->get_user_id()); # log activities
		...
		$sql = "
		INSERT INTO pages
		(SUB_CATEGORY_ID, SUB_SUB_CATEGORY_ID, PAGE_TITLE, PAGE_DETAILS, CREATED_BY, CREATED_DATE,STATUS)
		VALUES
		( ?, ?, ?, ?, ?, ?, 7)";
		$this->db->query($sql, array($input->sub_category_id, $input->sub_sub_category_id, addslashes($_POST['page_title']), addslashes($_POST['page_details']), $this->webspice->get_user_id(),$this->webspice->now()));
