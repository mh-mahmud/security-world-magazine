...
class Webspice{
	...
	function settings(){
		...
		$settings->site_title = 'The Security World';
		$settings->domain_name = 'The Security World';
		$settings->site_slogan = '';
		$settings->site_url = 'http://localhost/security_world/';
		$settings->site_url_prefix = '/security_world/';
	...
	*/
	...
	function get_path($param=null){
		...
		switch($param){
			...
			case 'personnel_full':
				...
				break;
			case 'small':
				return $url_prefix.'global/custom_files/small/';
				break;
			case 'small_full':
				return FCPATH.'global/custom_files/small/';
				break;
			case 'banner':
				return $url_prefix.'global/custom_files/banner/';
				break;
			case 'banner_full':
				return FCPATH.'global/custom_files/banner/';
				break;
			case 'staff':
	...
	function process_image_single($content_name, $image_name, $image_path_prefix, $image_width=null, $image_height=null, $process_function='thumb', $master_dim=null){
		if( $_FILES[$content_name]['tmp_name'] ){
			...
			$new_file_name = $this->get_path($image_path_prefix).$image_name;
